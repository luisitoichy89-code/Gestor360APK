...
# Se busca junto al propio archivo app.py para que funcione sin importar
# cuál sea el directorio de trabajo actual (relevante dentro del APK Android).
try:
...
#   SUPABASE_KEY=sb_publishable_xxxx   <- IMPORTANTE: usar la key PUBLISHABLE/ANON aquí,
#                                          nunca la sb_secret_/service_role en esta versión cliente,
#                                          ya que este código viaja dentro del APK de cada negocio.
# NOTA: ya NO hay CLIENTE_ID embebido aquí. Este mismo APK sirve para todos los
# negocios; el cliente_id correspondiente se descubre automáticamente la primera
# vez que el dispositivo se conecta, buscando su device_id en Supabase, y queda
# guardado en el SQLite local de ese teléfono (ver _obtener_cliente_id()).
...
CREATE TABLE IF NOT EXISTS usuarios ( 
    ...
    auth_id TEXT DEFAULT '', last_online_login TEXT DEFAULT '');
...
def _obtener_cliente_id():
    """Lee el cliente_id ya descubierto y guardado localmente para este
    dispositivo (config_local). Devuelve '' si todavía no se ha activado."""
    try:
        db = sqlite3.connect(DB_PATH, timeout=10)
        row = db.execute("SELECT valor FROM config_local WHERE clave='cliente_id'").fetchone()
        db.close()
        return row[0] if row else ''
    except Exception:
        return ''

def _guardar_cliente_id(cliente_id):
    db = sqlite3.connect(DB_PATH, timeout=10)
    db.execute("INSERT OR REPLACE INTO config_local (clave,valor) VALUES ('cliente_id',?)", (cliente_id,))
    db.commit(); db.close()

def _sync_desde_nube():
    """Descarga usuarios, locales, licencias y productos de este negocio desde
    Supabase hacia el SQLite local. Se usa tanto en /sync (manual, con sesión)
    como en /inicio (automático, primer arranque sin sesión todavía).
    El cliente_id se lee de config_local (descubierto vía device_id), no de
    una variable de entorno: este mismo APK sirve a cualquier negocio."""
    cliente_id = _obtener_cliente_id()
    if not (supabase and cliente_id):
        raise RuntimeError('Supabase no configurado o este dispositivo aún no fue activado')
    db=sqlite3.connect(DB_PATH, timeout=10)
    res=supabase.table("usuarios").select("*").eq("cliente_id", cliente_id).execute()
    if res.data:
        for u in res.data:
            # IMPORTANTE: no tocamos la columna 'password' aquí. Supabase ya no
            # guarda contraseñas (las maneja Auth); el hash local lo escribe
            # el propio flujo de login la primera vez que valida online.
            existing = db.execute("SELECT password FROM usuarios WHERE id=?", (u['id'],)).fetchone()
            pw_local = existing[0] if existing else ''
            db.execute("INSERT OR REPLACE INTO usuarios (id,username,password,nombre,rol,pin,almacen_id,activo,auth_id) VALUES (?,?,?,?,?,?,?,?,?)",
                       (u['id'],u['username'],pw_local,u.get('nombre'),u['rol'],u.get('pin'),u.get('almacen_id'),u.get('activo',1),u.get('auth_id','')))
            db.commit()
    res_loc=supabase.table("locales").select("*").eq("cliente_id", cliente_id).execute()
    if res_loc.data:
        for l in res_loc.data:
            db.execute("INSERT OR REPLACE INTO locales (id,nombre,activo,created_at) VALUES (?,?,?,?)",
                       (l['id'],l['nombre'],l.get('activo',1),l.get('created_at','')))
            db.commit()
    res_lic=supabase.table("licencias").select("*").eq("cliente_id", cliente_id).execute()
    if res_lic.data:
        for lc in res_lic.data:
            db.execute("INSERT OR REPLACE INTO licencias (id,device_id,almacen_id,expiracion,activo,created_at) VALUES (?,?,?,?,?,?)",
                       (lc['id'],lc['device_id'],lc.get('almacen_id'),lc.get('expiracion'),lc.get('activo',1),lc.get('created_at','')))
            db.commit()
    res2=supabase.table("productos").select("*").eq("cliente_id", cliente_id).execute()
    if res2.data:
        for p in res2.data:
...
color:#202124;font-family:'Google Sans','Segoe UI',Roboto,sans-serif;min-height:100vh;font-size:17px;overflow-x:hidden}
.sb-overlay{display:none;position:fixed;inset:0;background:rgba(32,33,36,0.5);z-index:499;backdrop-filter:blur(2px)}
...
box-shadow:4px 0 28px rgba(60,64,67,0.25)}
.sidebar.on{left:0}
.sb-head{padding:26px 18px 14px;background:#4285F4;border-bottom:1px solid #3367D6}
.sb-logo{font-size:20px;font-weight:800;color:#FFFFFF;letter-spacing:-.3px}
.sb-user{font-size:13px;color:#E8F0FE;margin-top:3px;font-weight:500}
.sb-section{padding:14px 18px 4px;font-size:10px;font-weight:800;
color:#4285F4;text-transform:uppercase;letter-spacing:2px}
.sb-item{display:flex;align-items:center;gap:13px;padding:13px 18px;
color:#5F6368;font-size:14px;font-weight:600;cursor:pointer;
border-left:3px solid transparent;transition:all .15s}
.sb-item:active,.sb-item:hover{background:#E8F0FE;border-left-color:#4285F4;color:#202124}
.sb-item .sbi{font-size:18px;width:24px;text-align:center;flex-shrink:0}
.sb-sep{height:1px;background:#DADCE0;margin:6px 0}
.topbar{display:flex;align-items:center;gap:10px;
margin:12px 14px 16px;padding:11px 13px;
background:#4285F4;border-radius:12px;border:1px solid #3367D6;box-shadow:0 2px 8px rgba(66,133,244,0.3)}
...
.hbg:active{background:rgba(255,255,255,0.15)}
.hbg span{display:block;width:100%;height:2.5px;background:#FFFFFF;border-radius:2px}
.tb-title{flex:1;font-weight:700;font-size:15px;color:#FFFFFF;
white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.tb-back{background:rgba(255,255,255,0.15);border:1px solid rgba(255,255,255,0.3);box-shadow:none;
width:auto;padding:7px 13px;font-size:13px;margin:0;border-radius:9px;
color:#FFFFFF;cursor:pointer;white-space:nowrap;flex-shrink:0;font-family:inherit}
.tb-back:active{background:rgba(255,255,255,0.25)}
.badge{background:rgba(255,255,255,0.18);border:1px solid rgba(255,255,255,0.4);color:#FFFFFF;
padding:2px 8px;border-radius:20px;font-size:11px;margin-left:4px;font-weight:700}
.box{background:#FFFFFF;border-radius:18px;
padding:32px 24px 22px;max-width:420px;width:90%;
border:1px solid #DADCE0;box-shadow:0 8px 32px rgba(60,64,67,0.18);margin:auto}
h1{text-align:center;margin-bottom:20px;font-size:26px;font-weight:800;color:#4285F4}
h2{margin:14px 14px 7px;font-size:19px;font-weight:700;color:#202124}
h3{margin:12px 14px 7px;font-size:16px;font-weight:700;color:#5F6368}
input,select{width:100%;padding:12px 14px;margin:6px 0;border-radius:10px;
border:1.5px solid #DADCE0;background:#FFFFFF;
color:#202124;font-size:16px;
outline:none;transition:all .2s;-webkit-appearance:none;font-family:inherit}
input:focus,select:focus{border-color:#4285F4;box-shadow:0 0 0 3px rgba(66,133,244,0.16);background:#FFFFFF}
...
button:active::after{background:rgba(0,0,0,0.04)}
button[type=submit],button.primary,.btn-primary{
background:#4285F4;color:#FFFFFF;border-color:#4285F4;
box-shadow:0 4px 14px rgba(66,133,244,0.35);font-weight:800}
button[type=submit]:active,button.primary:active{background:#3367D6}
button.dark{background:#F1F3F4;border-color:#DADCE0;color:#5F6368;box-shadow:none}
button.dark:active{background:#E8EAED}
button.danger{background:#FCE8E6;border-color:#EA4335;color:#C5221F;box-shadow:none}
button.danger:active{background:#FAD2CE}
button.success-btn{background:#E6F4EA;border-color:#34A853;color:#1E8E3E;box-shadow:none}
button.success-btn:active{background:#CEEAD6}
button.ghost{background:transparent;border:1.5px solid #DADCE0;color:#5F6368;box-shadow:none}
button.del{background:none;border:none;box-shadow:none;color:#EA4335;
...
.card{background:#FFFFFF;border-radius:14px;padding:18px 10px;
text-align:center;cursor:pointer;border:1px solid #DADCE0;
transition:all .18s;position:relative;overflow:hidden;box-shadow:0 1px 4px rgba(60,64,67,0.1)}
.card::before{content:'';position:absolute;top:0;left:0;right:0;height:2.5px;
background:#4285F4;border-radius:14px 14px 0 0;opacity:.8}
.card:active{transform:scale(.95);border-color:#4285F4;background:#E8F0FE}
.card .icon{font-size:30px;margin-bottom:7px}
.card .title{font-size:12px;font-weight:700;color:#5F6368;line-height:1.3}
.form-group{margin:0 14px 10px}
.form-group label{font-size:11px;font-weight:700;color:#5F6368;display:block;
margin-bottom:4px;text-transform:uppercase;letter-spacing:.8px}
.product-item{background:#FFFFFF;border:1px solid #DADCE0;
border-radius:12px;padding:13px 14px;margin:6px 14px;
display:flex;justify-content:space-between;align-items:center;transition:all .16s;box-shadow:0 1px 3px rgba(60,64,67,0.08)}
.product-item:active{background:#E8F0FE;border-color:#4285F4}
.product-item .name{font-weight:700;font-size:15px;color:#202124}
.product-item .info{color:#5F6368;font-size:13px;margin-top:3px}
.cart-item{background:#FFFFFF;border:1px solid #DADCE0;
border-radius:10px;padding:11px 13px;margin:5px 14px;
display:flex;justify-content:space-between;align-items:center;
font-size:15px;color:#202124;box-shadow:0 1px 3px rgba(60,64,67,0.08)}
.modal{display:none;position:fixed;top:0;left:0;right:0;bottom:0;
background:rgba(32,33,36,0.6);backdrop-filter:blur(8px);
...
box-shadow:0 12px 40px rgba(60,64,67,0.3);display:none}
.confirm-box .cf-title{font-size:16px;font-weight:800;color:#202124;margin-bottom:6px}
.confirm-box .cf-detail{font-size:13px;color:#5F6368;margin-bottom:16px;line-height:1.5}
.cf-btns{display:flex;gap:9px}
.cf-btns button{flex:1;margin:0}
.overlay{display:none;position:fixed;inset:0;background:rgba(32,33,36,0.45);z-index:599}
.overlay.active{display:block}
.toast{position:fixed;bottom:22px;left:14px;right:14px;
background:#202124;border:1px solid #3C4043;
color:#FFFFFF;padding:13px 16px;border-radius:12px;text-align:center;
z-index:700;font-weight:700;font-size:15px;opacity:0;
transition:opacity .2s;box-shadow:0 6px 20px rgba(60,64,67,0.4);pointer-events:none}
.toast.show{opacity:1}
.toast.tok{background:#1E8E3E;border-color:#1E8E3E;color:#FFFFFF}
.toast.terr{background:#C5221F;border-color:#C5221F;color:#FFFFFF}
.toast.twarn{background:#F29900;border-color:#F29900;color:#FFFFFF}
.error{background:#FCE8E6;border:1px solid #EA4335;color:#C5221F;
...
.stat-card{background:#FFFFFF;border:1px solid #DADCE0;border-radius:12px;padding:13px;text-align:center;box-shadow:0 1px 4px rgba(60,64,67,0.1)}
.stat-val{font-size:19px;font-weight:800;color:#4285F4}
.stat-lbl{font-size:11px;color:#5F6368;margin-top:3px;font-weight:600;text-transform:uppercase;letter-spacing:.4px}
.inv-header{background:#FFFFFF;border:1px solid #DADCE0;border-radius:12px;padding:14px;margin:7px 14px 3px;box-shadow:0 1px 4px rgba(60,64,67,0.1)}
.lic-card{background:#FFFFFF;border:1px solid #DADCE0;border-radius:12px;padding:13px 14px;margin:6px 14px;box-shadow:0 1px 4px rgba(60,64,67,0.1)}
.lic-id{font-family:monospace;font-size:13px;color:#4285F4;word-break:break-all;
background:#E8F0FE;border-radius:7px;padding:7px 9px;margin:5px 0}
.lic-meta{font-size:12px;color:#5F6368;margin-top:4px}
.s-ok{color:#1E8E3E;font-weight:700}.s-err{color:#C5221F;font-weight:700}
.device-box{background:#FFFFFF;border:2px dashed #4285F4;border-radius:13px;padding:18px;margin:14px 0;text-align:center}
.device-id-txt{font-family:monospace;font-size:16px;font-weight:800;color:#4285F4;
letter-spacing:1px;word-break:break-all;
background:#E8F0FE;border-radius:8px;padding:10px;margin:10px 0}
.pin-display{display:flex;justify-content:center;gap:11px;margin:14px 0}
.pin-dot{width:14px;height:14px;border-radius:50%;border:2px solid #DADCE0;transition:all .17s}
.pin-dot.filled{background:#4285F4;border-color:#4285F4;box-shadow:0 0 8px rgba(66,133,244,0.5)}
...
.chat-bubble.me{background:#4285F4;color:#FFFFFF;margin-left:auto;border-radius:13px 13px 3px 13px;font-weight:600}
.chat-bubble.other{background:#FFFFFF;border:1px solid #DADCE0;color:#202124;border-radius:13px 13px 13px 3px}
.bill-row{display:flex;justify-content:space-between;align-items:center;
background:#FFFFFF;border:1px solid #DADCE0;
border-radius:10px;padding:10px 14px;margin:5px 14px;box-shadow:0 1px 3px rgba(60,64,67,0.08)}
.bill-denom{font-size:16px;font-weight:800;min-width:68px;color:#5F6368}
.bill-input{width:76px;font-size:17px;text-align:center;padding:7px;margin:0}
.bill-sub{min-width:86px;text-align:right;font-weight:700;color:#4285F4;font-size:14px}
.bill-total{color:#4285F4;font-size:26px;font-weight:800}
.page-btn{background:#FFFFFF;border:1px solid #DADCE0;
padding:8px 14px;border-radius:8px;margin:2px;cursor:pointer;
font-weight:700;font-size:13px;color:#5F6368;width:auto;box-shadow:none;font-family:inherit}
.page-btn.active{background:#4285F4;color:#FFFFFF;border-color:#4285F4}
.prod-actions{background:#FFFFFF;border:1px solid #DADCE0;border-radius:12px;margin:0 14px 12px;padding:15px;box-shadow:0 1px 4px rgba(60,64,67,0.1)}
...
border-top:1px solid #DADCE0;font-size:11px;color:#9AA0A6;line-height:1.9}
.footer-login a{color:#4285F4;text-decoration:none}
.footer{background:#FFFFFF;color:#9AA0A6;text-align:center;
padding:14px;margin-top:28px;font-size:12px;border-top:1px solid #DADCE0}
...
.top5-wrap{background:#E8F0FE;border:1px solid #C2D7FB;border-radius:13px;padding:12px;margin:0 14px 10px}
.top5-title{font-size:11px;font-weight:800;color:#4285F4;text-transform:uppercase;letter-spacing:1px;margin-bottom:8px}
...
.top5-chip{flex-shrink:0;background:#FFFFFF;border:1px solid #4285F4;border-radius:11px;padding:9px 13px;cursor:pointer;transition:all .15s;min-width:100px;max-width:150px}
.top5-chip:active{transform:scale(.95);background:#E8F0FE}
.top5-chip .tc-name{font-size:12px;font-weight:700;color:#202124;white-space:nowrap;overflow:hidden;text-overflow:ellipsis}
.top5-chip .tc-meta{font-size:10px;color:#5F6368;margin-top:2px}
.top5-chip .tc-rank{font-size:9px;color:#4285F4;font-weight:800;margin-top:3px}
"""
TOAST_JS = """
...
def inicio():
    ...
    db_raw = sqlite3.connect(DB_PATH, timeout=10); db_raw.row_factory = sqlite3.Row
    lic = db_raw.execute("SELECT * FROM licencias WHERE device_id=? AND activo=1", (dev_id,)).fetchone()

    # Si no hay licencia local todavía (primer arranque, o el superadmin
    # acaba de activarla en Supabase), buscamos por device_id en la nube.
    # Esto evita el candado de "sin licencia no entra, sin login no sincroniza":
    # en cuanto encontramos la licencia, descubrimos también a qué negocio
    # (cliente_id) pertenece este dispositivo, lo guardamos localmente, y
    # sincronizamos usuarios/locales/productos para que el login ya funcione.
    # Este mismo APK sirve para cualquier negocio: no hay cliente_id de fábrica.
    if not lic and supabase:
        try:
            res = supabase.table("licencias").select("*").eq("device_id", dev_id).execute()
            if res.data:
                lc = res.data[0]
                _guardar_cliente_id(lc['cliente_id'])
                _sync_desde_nube()
                lic = db_raw.execute("SELECT * FROM licencias WHERE device_id=? AND activo=1", (dev_id,)).fetchone()
        except Exception:
            pass  # sin conexión: seguimos con lo que haya local (o nada)
    db_raw.close()
    ...
    if request.method == 'POST':
        ...
        if is_account_locked(u):
            ...
            if not user:
                ...
                login_ok = False
                modo = ''
                # 1) Intentamos validar contra Supabase Auth (requiere internet).
                #    Esto es lo que realmente autentica al usuario y lo que
                #    permite que RLS funcione correctamente más adelante.
                cliente_id = _obtener_cliente_id()
                if supabase and cliente_id:
                    try:
                        synth_email = u + '@' + cliente_id[:8] + '.gestor360.local'
                        auth_res = supabase.auth.sign_in_with_password({"email": synth_email, "password": p})
                        if auth_res.user:
                            login_ok = True
                            modo = 'online'
                            # Actualizamos la sesión cacheada local con este password
                            # válido, para poder seguir entrando offline hasta 30 días.
                            pw_hash = hashlib.sha256(p.encode()).hexdigest()
                            ahora = datetime.datetime.now().isoformat()
                            db.execute("UPDATE usuarios SET password=?, last_online_login=?, auth_id=? WHERE id=?",
                                       (pw_hash, ahora, auth_res.user.id, user['id']))
                            db.commit()
                    except Exception:
                        pass  # sin red o credenciales inválidas online: probamos modo offline abajo
                # 2) Si no se pudo validar online (sin red, o Supabase no configurado),
                #    caemos a la sesión cacheada localmente, válida hasta 30 días desde
                #    el último login online exitoso.
                if not login_ok and user['password']:
                    if user['password'] == hashlib.sha256(p.encode()).hexdigest():
                        vencida = False
                        if user['last_online_login']:
                            try:
                                ultima = datetime.datetime.fromisoformat(user['last_online_login'])
                                vencida = (datetime.datetime.now() - ultima).days > 30
                            except Exception:
                                vencida = False
                        if vencida:
                            error = '<div class="error">⚠️ Sesión offline vencida (30 días). Conecta a internet para renovar tu acceso.</div>'
                        else:
                            login_ok = True
                            modo = 'offline'
                ...
                if login_ok:
                    session.clear()
                    session['user'] = dict(db.execute("SELECT * FROM usuarios WHERE id=?", (user['id'],)).fetchone())
                    register_success_login(u)
                    add_trace(u,'LOGIN','Inicio exitoso ('+modo+')',str(user['almacen_id'] or '1'))
                    log_security('LOGIN_OK', 'usuario=' + u + ' modo=' + modo, ip)
...
def dashboard():
    ...
    if rol == 'admin':
        ...
        if len(alms) > 1:
            top += '<button class="tb-back" style="padding:7px 11px" onclick="document.getElementById(\'mLocalSel\').classList.add(\'active\')">🏪 Local ' + a + ' ▾</button>'
    top += '</div></div>'
    if rol == 'admin' and len(get_almacenes_usuario()) > 1:
        opts_html = ''
        for al in get_almacenes_usuario():
            sel_cls = 'background:#4285F4;color:#FFFFFF;border-color:#4285F4' if al == a else ''
            opts_html += '<button style="' + sel_cls + '" onclick="location.href=\'/cambiar_local/' + al + '\'">🏪 Local ' + al + (' ✓' if al == a else '') + '</button>'
        top += ('<div class="modal" id="mLocalSel" onclick="if(event.target===this)this.classList.remove(\'active\')">'
                '<div class="box" style="margin-top:20vh">'
                '<h2 style="color:#202124">Cambiar de local</h2>'
                + opts_html +
                '<button class="dark" onclick="document.getElementById(\'mLocalSel\').classList.remove(\'active\')">Cancelar</button>'
                '</div></div>')
...
@app.route('/tarjetas')
def tarjetas():
    if 'user' not in session or session['user'].get('rol') != 'admin': return redirect(url_for('dashboard'))
...
def trazas():
    ...
    h+='<div style="padding:0 14px;display:flex;gap:8px"><button onclick="tFilt()">🔍 Filtrar</button></div>'
    ...
    if tp>1:
        ...
        h+='</div>'
    h+=('<script>'
         'function tFilt(){var f=document.getElementById("tF").value;location.href="/trazas?fecha="+f}'
         'function tPage(p){var f=document.getElementById("tF").value;location.href="/trazas?page="+p+"&fecha="+f}'
         '</script>')
...
.chat-av{width:28px;height:28px;border-radius:50%;background:#E8F0FE;color:#4285F4;
display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:800;flex-shrink:0}
.chat-row.me .chat-av{background:#4285F4;color:#FFFFFF}
...
.chat-bubble.me{background:#4285F4;color:#FFFFFF;
...
#cInp:focus{border-color:#4285F4;outline:none}
#cSendBtn{width:44px;height:44px;min-width:44px;border-radius:50%;margin:0;padding:0;
display:flex;align-items:center;justify-content:center;font-size:18px;flex-shrink:0;
background:#4285F4;color:#FFFFFF;border:none;box-shadow:0 2px 8px rgba(66,133,244,.35)}
...
@app.route('/sync')
def sync():
    if session.get('user',{}).get('rol') != 'admin': return redirect(url_for('dashboard'))
    if not _obtener_cliente_id():
        return page_auth("Sync", topbar("Sync","/dashboard")+'<div class="error">⚠️ Este dispositivo aún no fue activado. Contacta al proveedor.</div><div style="padding:0 14px"><button onclick="location.href=\'/dashboard\'">← Dashboard</button></div>')
    try:
        _sync_desde_nube()
        add_trace(session['user']['username'],'SYNC','Exitosa', get_almacen())
...
@app.route('/seguridad')
def seguridad():
    return pol("Seguridad","<h2>Política de Seguridad</h2><p>Actualización: 20/06/2026</p><br><h3>1. Protección</h3><p>SHA256, rate limiting 5 intentos/5min por IP, bloqueo de cuenta tras 10 intentos fallidos (30 min), backup cada hora, cabeceras HTTP de seguridad.</p><br><h3>2. Roles</h3><p>Admin (Jefe, PIN 5 min) / Vendedor.</p><br><h3>3. Sincronización</h3><p>Los datos del negocio se administran de forma centralizada y se sincronizan a este dispositivo de forma segura.</p><br><h3>4. Contacto</h3><p>luisi26@nauta.cu</p>")
...
def licencia_terminos():
    return pol("Licencia","<h2>Acuerdo de Licencia</h2><p>Actualización: 14/06/2026</p><br><h3>1. Modelo</h3><p>Licencia por dispositivo. No transferible.</p><br><h3>2. Vigencia</h3><p>Definida por el proveedor del servicio en días.</p><br><h3>3. Contacto</h3><p>luisi26@nauta.cu</p>")
