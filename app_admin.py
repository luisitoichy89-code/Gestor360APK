...
#   SUPABASE_KEY=sb_secret_xxxx   <- ESTE panel SÍ usa la key SECRET/service_role,
#                                     porque corre solo en TU Termux y nunca se
#                                     distribuye a clientes. Necesita bypass de RLS
#                                     para poder gestionar todos los negocios.
...
def get_negocio_activo():
    """ID (uuid) del negocio/cliente con el que el superadmin está trabajando
    ahora mismo en el panel. Se guarda en sesión vía /seleccionar_negocio."""
    return session.get('negocio_activo', '')
...
def negocios():
    ...
    if not supabase:
        return page_auth("Negocios", topbar("Negocios","/dashboard")+'<div class="error">⚠️ Supabase no configurado.</div>')
    try:
        res = supabase.table("clientes").select("*").order("created_at", desc=True).execute()
        negs = res.data or []
    except Exception as e:
        negs = []
    activo_id = get_negocio_activo()
    h = topbar("Negocios", "/dashboard")
    h += '<h3>Nuevo Negocio</h3>'
    h += '<div class="form-group"><label>Nombre del negocio</label><input type="text" id="ngNom" placeholder="Ej: Cafetería La Esquina"></div>'
    h += '<div style="padding:0 14px"><button onclick="confirmar(\'¿Crear negocio?\',\'\',function(ok){if(ok)crearNegocio()})">🏢 Crear Negocio</button></div>'
    h += '<h3>Negocios (' + str(len(negs)) + ')</h3>'
    for n in negs:
        es_activo = n['id'] == activo_id
        h += ('<div class="lic-card" style="' + ('border-color:#4285F4' if es_activo else '') + '">'
              '<div style="display:flex;justify-content:space-between;align-items:flex-start">'
              '<div><div style="font-weight:700;font-size:15px;color:#E8EAF6">🏢 '+str(n['nombre_negocio'])+(' ✅' if es_activo else '')+'</div>'
              '<div class="lic-meta">ID: '+str(n['id'])+'</div>'
              '<div class="lic-meta">'+('🟢 Activo' if n.get('activo',True) else '🔴 Inactivo')+'</div></div>'
              '<button class="btn-sm dark" onclick="seleccionarNegocio(\''+str(n['id'])+'\')">'+('Seleccionado' if es_activo else 'Seleccionar')+'</button>'
              '</div></div>')
    h += ('<script>'
          'function crearNegocio(){var n=document.getElementById("ngNom").value.trim();'
          'if(!n){toast("Nombre obligatorio","terr");return}'
          'fetch("/api/crear_negocio",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({nombre:n})})'
          '.then(r=>r.json()).then(d=>{if(d.ok){toast("Negocio creado","tok");location.reload()}else toast(d.error,"terr")})}'
          'function seleccionarNegocio(id){location.href="/seleccionar_negocio/"+id}'
          '</script>')
    return page_auth("Negocios", h)

@app.route('/api/crear_negocio', methods=['POST'])
def api_crear_negocio():
    if session.get('user',{}).get('rol') != 'superadmin': return jsonify({'ok':False})
    if not supabase: return jsonify({'ok':False,'error':'Supabase no configurado'})
    d = request.json or {}
    nombre = clean_text(d.get('nombre',''), 150)
    if not nombre: return jsonify({'ok':False,'error':'Nombre obligatorio'})
    try:
        res = supabase.table("clientes").insert({"nombre_negocio": nombre, "activo": True}).execute()
        log_security('CREAR_NEGOCIO', nombre, request.remote_addr)
        return jsonify({'ok':True, 'id': res.data[0]['id'] if res.data else None})
    except Exception as e:
        return jsonify({'ok':False,'error':str(e)[:150]})

@app.route('/seleccionar_negocio/<negocio_id>')
def seleccionar_negocio(negocio_id):
    if session.get('user',{}).get('rol') != 'superadmin': return redirect(url_for('dashboard'))
    session['negocio_activo'] = negocio_id
...
def api_crear_local():
    if session.get('user',{}).get('rol')!='superadmin': return jsonify({'ok':False})
    negocio_id = get_negocio_activo()
    if not negocio_id: return jsonify({'ok':False,'error':'Selecciona un negocio activo primero (menú → Negocios)'})
    if not supabase: return jsonify({'ok':False,'error':'Supabase no configurado'})
    d=request.json or {}; db=get_db()
    nombre = clean_text(d.get('nombre',''), 100)
    if not nombre: return jsonify({'ok':False,'error':'Nombre obligatorio'})
    try:
        res = supabase.table("locales").insert({"cliente_id": negocio_id, "nombre": nombre, "activo": True}).execute()
        new_id = res.data[0]['id'] if res.data else None
        if new_id is not None:
            db.execute("INSERT OR REPLACE INTO locales (id,nombre,activo,created_at) VALUES (?,?,1,datetime('now'))",(new_id,nombre))
            db.commit()
...
def licencias():
    ...
    if total_pages > 1:
        ...
        h += '</div>'
    h+=('<div class="modal" id="mRenovar" onclick="if(event.target===this)this.classList.remove(\'active\')">'
        '<div class="box" style="margin-top:25vh">'
        '<h2 style="color:#202124">Renovar licencia</h2>'
        '<div class="form-group"><label>Días a renovar</label><input type="number" id="rnDias" value="30" min="1"></div>'
        '<button onclick="confirmarRenovacion()">Renovar</button>'
        '<button class="dark" onclick="document.getElementById(\'mRenovar\').classList.remove(\'active\')">Cancelar</button>'
        '</div></div>')
    h+=('<script>'
         ...
         'var rnDev="",rnAlm="";'
         'function renovar(dev,alm){rnDev=dev;rnAlm=alm;document.getElementById("rnDias").value="30";document.getElementById("mRenovar").classList.add("active")}'
         'function confirmarRenovacion(){var dias=document.getElementById("rnDias").value;'
         'if(!dias||isNaN(parseInt(dias))||parseInt(dias)<1){toast("Días inválidos","terr");return}'
         'document.getElementById("mRenovar").classList.remove("active");'
         'fetch("/api/add_licencia",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({device_id:rnDev,almacen_id:rnAlm,dias:dias})})'
...
def api_add_licencia():
    ...
    exp=(datetime.datetime.now()+datetime.timedelta(days=dias)).strftime('%Y-%m-%d') if dias>0 else None
    try:
        existing = supabase.table("licencias").select("id").eq("device_id", device_id).execute()
        payload = {"cliente_id": negocio_id, "device_id": device_id, "almacen_id": almacen_id, "expiracion": exp, "activo": True}
        if existing.data:
            supabase.table("licencias").update(payload).eq("device_id", device_id).execute()
        else:
            supabase.table("licencias").insert(payload).execute()
        db.execute("INSERT OR REPLACE INTO licencias (device_id,almacen_id,expiracion,activo,created_at) VALUES (?,?,?,1,datetime('now'))",
                   (device_id,almacen_id,exp or ''))
        db.commit()
...
def usuarios():
    ...
    h+=('<h3>➕ Crear Usuario</h3>'
         ...
         '<div class="form-group"><label>Contraseña (mínimo 6 caracteres)</label><input type="password" id="nPw" placeholder="Contraseña de acceso"></div>'
    ...
    h+=('<script>'
         ...
         'r=document.getElementById("nR").value,p=document.getElementById("nP").value,pw=document.getElementById("nPw").value;'
         'var a_str=document.getElementById("nA").value.trim();'
         'if(!u||!a_str){toast("Usuario y al menos un local son obligatorios","terr");return}'
         'if(!pw||pw.length<6){toast("Contraseña mínimo 6 caracteres","terr");return}'
         'var res=await fetch("/api/crear_usuario",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({username:u,nombre:n,rol:r,almacen_id:a_str,pin:p,password:pw})});'
...
def crear_usuario():
    ...
    if not password or len(password) < 6:
        return jsonify({'ok':False,'error':'Contraseña obligatoria (mínimo 6 caracteres)'})
    # Email sintético único: el negocio + username no chocan entre clientes
    # distintos porque cada negocio tiene su propio cliente_id en el dominio.
    synth_email = username + '@' + negocio_id[:8] + '.gestor360.local'
    try:
        ...
        ins = supabase.table("usuarios").insert({
            "auth_id": auth_id,
        ...
        if new_id is not None:
            db.execute(
                "INSERT OR REPLACE INTO usuarios (id,username,password,nombre,rol,pin,almacen_id,activo) VALUES (?,?,?,?,?,?,?,1)",
                (new_id, username, hashlib.sha256(password.encode()).hexdigest(), nombre, rol, pin, almacen_id))
            db.commit()
        add_trace(session['user']['username'], 'CREAR USUARIO',
                  username + ' · ' + rol, str(session['user'].get('almacen_id','1')))
        log_security('CREAR_USUARIO', 'username='+username+' rol='+rol, request.remote_addr)
        return jsonify({'ok': True})
    except Exception as e:
        return jsonify({'ok': False, 'error': 'No se pudo crear el usuario: '+str(e)[:150]})
